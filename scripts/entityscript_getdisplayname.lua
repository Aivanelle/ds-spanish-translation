...
if oldGetDisplayName and anyDLCEnabled then
	function EntityScript:GetDisplayName(...)
		...
		if showAdjectives and ((isWet or self.always_wet) and not self.no_wet_prefix) then
			if self.components.edible and GetPlayer() and GetPlayer().components.eater and GetPlayer().components.eater:CanEat(self) then
			...
			--[[
				if PREFABS.NAMED_LIVING_BEING[prefab] then
					local livingBeingName = self.components.named.name
					local wetLivingBeingPrefix = Prefix.getWetLivingBeingPrefix(livingBeingName)

						Accented names may cause some sort of encoding troubles, if that happens it will return just the name
						without any adjective.

					if wetLivingBeingPrefix then return ConstructAdjectivedName(self, name, wetLivingBeingPrefix) end
					return livingBeingName
				end
			]]
				if self.recipetouse then return name:gsub("Planos", "Planos húmedos") end
				...
				if self.prefab == "pigskin" and name:find("?") then return name:gsub("?", " " .. wetGenericPrefix .. "?") end
